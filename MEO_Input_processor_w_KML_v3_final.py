...
def singleburst_loc(df, lat_GT, lon_GT, MEOLUT):
    ...
    ##df2['LAT_e_rad'] = None if df2.Enc_Lat.isnull else np.radians(df2['Enc_Lat'])
    ##df2['LAT_e_rad'] = df2[['Enc_Lat','Enc_Lon']].apply(my_func, axis = 1)  ### WORKS
    #df2['LAT_e_rad'] = df2[['Enc_Lat']].apply(lambda x: np.radians(x) if pd.notnull(x[0]) else x, axis = 1)
    #df2['LON_e_rad'] = df2[['Enc_Lon']].apply(lambda x: np.radians(x) if pd.notnull(x[0]) else x, axis = 1)
    #print df2[['Enc_Lat','Enc_Lon','LAT_e_rad','LON_e_rad']]
    ...
    df2['Error_GT'] = df2[['Latitude','Longitude', 'Lat_GT','Lon_GT']].apply(haversine2, axis = 1)
    #print df2[['BcnId30', 'Enc_Lat','dLAT_e','Error_GT']].head(20)
    #print df2.Enc_Lat.isnull()

    #Enc_Error = 6373 * 2 * np.arcsin(np.sqrt(np.sin(df2['dLAT_e']/2)**2 + np.cos(np.radians(df2['Enc_Lat'])) * np.cos(df2['LAT_rad']) * np.sin(df2['dLON_e']/2)**2))
    
    #print Enc_Error
    df2['Error_Enc'] = df2[['Latitude','Longitude','Enc_Lat','Enc_Lon']].apply(haversine2, axis = 1)
    #df2['Error_Enc'] = None if pd.isnull(df2.Enc_Lat) else 6373 * 2 * np.arcsin(np.sqrt(np.sin(df2['dLAT_e']/2)**2 + 
    #    np.cos(np.radians(df2['Enc_Lat'])) * np.cos(df2['LAT_rad']) * np.sin(df2['dLON_e']/2)**2))

    #print df2.Error_Enc.head(5)
...
def multiburst_loc(df,lat_GT,long_GT,MEOLUT, window_span):
    ...
    df2['Error_Enc'] = None if df2.Enc_Lat.isnull else 6373 * 2 * np.arcsin(np.sqrt(np.sin(df2['dLAT_e']/2)**2 + 
        np.cos(np.radians(df2['Enc_Lat'])) * np.cos(df2['LAT_rad']) * np.sin(df2['dLON_e']/2)**2))
...
if LEOGEO == 'Y':
    ...
    df = pd.read_excel(XLSXfolder +'\\' + LEOGEO_file, index_col = 'A_Tca', parse_dates = True)
        ...
        with open(CSVoutfolder +'\\' + LEOGEO_file + '.csv', 'rb') as csvfile:
            ...
            for row in filereader:            
                pnt_LEO = fol_LEO.newpoint(coords=[(float(row[22]),float(row[21]))], 
                    description = 'LEO Location - Beacon = ' + row[15] + '\n\nLUT = ' + row[2] + '\nSat = ' + row[3] +
