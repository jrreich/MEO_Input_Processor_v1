...
def singleburst_loc(df, lat_GT, long_GT, MEOLUT):
    ...
    dfSB = df2.drop(['LAT_rad','LON_rad','dLON','dLAT'],axis=1)
...
#bin1 = lambda x: bin(int(x,16))
#def country_code(df): 
#    return int(bin(int(str(df),16))[2:].zfill(len(str(df))*4)[1:11],2)
#def hex30tobin(df):
#    return bin(int(str(df),16))[2:].zfill(len(str(df))*4)
print df3['BcnId30']
df3['enc_lat'] = bcn.beacon.lat(df3['BcnId30'])
df3['enc_lon'] = bcn.beacon.lon(df2['BcnId30'])
print df3.head(15)
